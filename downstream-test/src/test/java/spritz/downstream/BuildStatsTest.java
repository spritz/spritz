...
public class BuildStatsTest
{
  ...
  private void compareSizesForBranch( @Nonnull final String branch, final boolean includeBranchInFixtureKey )
    throws Exception
  {
    final String nextVersion = SystemProperty.get( "spritz.next.version" );
    final String previousVersion = SystemProperty.get( "spritz.prev.version" );
    ...
    if ( 0 != nextVersionFixtureSize )
    {
      if ( nextVersionFixtureSize != afterSize )
      {
        ...
        fail( "Build size after upgrading spritz in branch '" + branch + "' does not match the build size " +
    ...
    else
    {
      ...
      {
        reportSymbolDifferences( beforeBuild, afterBuild );
        fail( "Build size changed after upgrading spritz in branch '" + branch + "' from " + beforeSize + " to " +
