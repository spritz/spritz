...
task 'update_api_diff' do
  ...
  sh "buildr clean spritz:api-test:test TEST=only GWT=no PRODUCT_VERSION=#{ENV['PRODUCT_VERSION']} PREVIOUS_PRODUCT_VERSION=#{ENV['PREVIOUS_PRODUCT_VERSION']} STORE_API_DIFF=true"
end
...
task 'test_api_diff' do
  ...
  sh "buildr clean spritz:api-test:test TEST=only GWT=no PRODUCT_VERSION=#{ENV['PRODUCT_VERSION']} PREVIOUS_PRODUCT_VERSION=#{ENV['PREVIOUS_PRODUCT_VERSION']}"
end

desc 'Regenerate api diffs'
task 'regenerate_api_diffs' do
  (2...6).each do |v|
    apidiff_generate("org.realityforge.spritz:spritz-core:jar", "0.0#{v}", "0.0#{v + 1}")
  end
end
